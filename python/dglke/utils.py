...
def save_model(args, model, emap_file=None, rmap_file=None):
    ...
    if hasattr(cloned_args, "wandb"):
        delattr(cloned_args, "wandb")
...
def load_raw_triplet_data(
    head_f=None, rel_f=None, tail_f=None, emap_f=None, rmap_f=None
):
...
class CommonArgParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument(
            "--batch_size", type=int, default=1024, help="The batch size for training."
        )
        self.add_argument(
            "--batch_size_eval",
            type=int,
