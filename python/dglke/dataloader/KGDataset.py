...
class KGDatasetWikikg2(KGDataset):
    ...
    * test.txt stores the triples in the test set.
    ...
    def __init__(self, path, name="wikikg2"):
        ...
        super(KGDatasetWikikg2, self).__init__(
            os.path.join(self.path, "entities.dict"),
            os.path.join(self.path, "relations.dict"),
            os.path.join(self.path, "train.txt"),
            os.path.join(self.path, "valid.txt"),
...
class KGDatasetBiokg(KGDataset):
    ...
    * test.txt stores the triples in the test set.
    ...
    def __init__(self, path, name="biokg"):
        ...
        super(KGDatasetBiokg, self).__init__(
            os.path.join(self.path, "entities.dict"),
            os.path.join(self.path, "relations.dict"),
            os.path.join(self.path, "train.txt"),
            os.path.join(self.path, "valid.txt"),
...
class KGDatasetUDDRaw(KGDataset):
    ...
    * test stores the triples in the test set. In format [src_name, rel_name, dst_name]
    ...
    def load_entity_relation(self, path, delimiter, files, format):
        entity_map = {}
        rel_map = {}
        entity_dict_file = os.path.join(path, "entities.tsv")
        rel_dict_file = os.path.join(path, "relations.tsv")

        if os.path.exists(entity_dict_file):
            print("Loading entity dictionary from {}".format(entity_dict_file))
            with open(entity_dict_file) as f:
                for line in f:
                    eid, entity = line.strip().split(delimiter)
                    entity_map[entity] = int(eid)

        if os.path.exists(rel_dict_file):
            print("Loading relation dictionary from {}".format(rel_dict_file))
            with open(rel_dict_file) as f:
                for line in f:
                    rid, rel = line.strip().split(delimiter)
                    rel_map[rel] = int(rid)

        if len(entity_map) == 0 or len(rel_map) == 0:
            print("No entity or relation dictionary found. Creating new dictionary.")
        ...
        if not os.path.exists(entity_dict_file):
            print("Creating a new entity dictionary at {}".format(entity_dict_file))
        ...
        if not os.path.exists(rel_dict_file):
            print("Creating a new relation dictionary at {}".format(rel_dict_file))
            with open(rel_dict_file, "w+") as f:
